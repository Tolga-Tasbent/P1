Shader "unityCookie/Intermediate/8 Vertex Animation"
{
	Properties{
	...
	SubShader{
		Pass{
			...
			vertexOutput vert(vertexInput v) {
				...
				newPos.xyz = newPos.xyz + sin(_Time.x * 10 * _AnimSpeed + (animOffset.x + animOffset.y + animOffset.z) * _AnimFreq) * animPower.xyz;
			...
			fixed4 frag(vertexOutput i) : COLOR{
				...
				fixed3 specularReflection = diffuseReflection * _SpecColor.xyz * 
							pow(saturate(dot(reflect(-i.lightDir.xyz, i.normalDir), 
